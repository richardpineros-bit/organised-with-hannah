...
// Uses relative path so nginx proxies /api to the backend server
const API_BASE = '/api';
...
class ApiClient {
  ...
  async register(email: string, password: string, name?: string, phone?: string) { return this.post('/auth/register', { email, password, name, phone }); }
  ...
  async updateContent(section: string, key: string, value: string, type = 'text') { return this.put(`/content/${section}/${key}`, { value, type }); }
  async bulkUpdateContent(data: Record<string, Record<string, any>>) { return this.post('/content/bulk', data); }

  // Services
  async getServices() { return this.get('/services'); }

  // Bookings
  async getAvailability(date: string) { return this.get(`/bookings/availability?date=${date}`); }
  async createBooking(data: any) { return this.post('/bookings', data); }
  ...
  // Quiz
  async getQuiz(slug: string) { return this.get(`/quiz/${slug}`); }
  async submitQuiz(slug: string, answers: any[], email?: string) { return this.post(`/quiz/${slug}/submit`, { answers, email }); }

  // Products
  async getProducts() { return this.get('/products'); }
  async getProduct(slug: string) { return this.get(`/products/${slug}`); }
}
